  ...
  return (
    <div className="grid grid-cols-2 gap-3 sm:grid-cols-4 lg:grid-cols-5" data-testid="cuti-balance">
      ...
      <div className="col-span-2 rounded-2xl border border-slate-800 bg-slate-900 p-4 text-white shadow-sm sm:col-span-4 lg:col-span-1">
        ...
        <p className="mt-1 text-[10px] text-emerald-300" data-testid="cuti-dipakai">Terpakai tahun ini: {b.dipakai ?? 0} hari</p>
        <p className="mt-0.5 text-[10px] text-slate-400">Prioritas: Bersama → N-2 → N-1 → N</p>
      </div>
    </div>
  );
}
          ...
          <tbody className="divide-y divide-slate-100" data-testid="saldo-table">
            {rows.map((b) => (
              <tr key={b.employee_id} className="hover:bg-slate-50/60">
                ...
                <td className="px-4 py-3"><span className="rounded-full bg-emerald-50 px-2.5 py-0.5 text-xs font-semibold text-emerald-700" data-testid={`saldo-dipakai-${b.employee_id}`}>{b.dipakai || 0} hari</span></td>
